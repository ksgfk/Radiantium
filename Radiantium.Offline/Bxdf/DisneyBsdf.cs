...
namespace Radiantium.Offline.Bxdf
{
    ...
    //I dont know the corrent implemention for Disney BSDF. Life is sooooooo hard :(
    ...
    //    {
    //        float dielectric = Fresnel.DielectricFunc(cosI, 1, Eta);
    //        Color3F conductor = Fresnel.ConductorFunc(cosI, new Color3F(1), new Color3F(0.4f), new Color3F(1.6f));
    //        return Lerp(Metallic, new Color3F(dielectric), R0 * conductor);
    ...
    //        Clearcoat = clearcoat;
    //        ClearcoatRoughness = Lerp(clearcoatGloss, 0.1f, 0);
    ...
    //        wh = Normalize(wh);
    //        DisneyFresnel fresnel = new(GetSpecColor(), Metallic, Ior);
    ...
    //        Color3F result = Abs((d * g * f) / (4 * cosThetaI * cosThetaO));
    //        return result;
    //    }
    ...
    //    {
    //        if (CosTheta(wo) == 0) { wi = default; return false; }
    //        DisneyDistributionGTR2 dist = new(TransAnisX, TransAnisy);
    //        Vector3 wh = dist.SampleWh(wo, rand);
    //        if (Dot(wo, wh) < 0) { wi = default; return false; }
    //        wi = Reflect(-wo, wh);
    //        return SameHemisphere(wo, wi);
    //    }
    ...
    //    private Color3F GetSpecColor()
    //    {
    //        return Lerp(Metallic, Specular * SchlickR0FromEta(Ior) * Lerp(SpecularTint, new Color3F(1.0f), ColorTint), BaseColor);
    //    }
